...
g_lik <- function(mcmc, data, i){
  ...
  if(mcmc$v < 0 | mcmc$mu < 0 | mcmc$p < 0 | mcmc$b < 0 | mcmc$b > 1 | mcmc$w[i] < 0){
    ...
    if(delta_t < 0){
      ...
      # log probability of iSNV in expo growth phase
      log_p_isnv <- log(1 - (1 - mcmc$p)^(1/sqrt(mcmc$p)))

      # log probability of no iSNV in expo growth phase
      log_p_no_isnv <- log(1 - mcmc$p) / sqrt(mcmc$p)
      ...
      if(i <= data$n_obs){
        if(!is.null(data$snvs[[i]]$isnv)){
          ...
          out <- out +
            ...
            sum(log(
              (1/4 + (3/4)*exp(-mcmc$mu * delta_t))*denovo(freq_0y, mcmc$p) + (1/4 - (1/4)*exp(-mcmc$mu * delta_t))*denovo(1 - freq_0y, mcmc$p)
            )) +

            # Likelihood from x = 1, 0 < y < 1
            length(mcmc$m1y[[i]]) * log_p_isnv +
            sum(log(
              (1/4 + (3/4)*exp(-mcmc$mu * delta_t))*denovo(1 - freq_1y, mcmc$p) + (1/4 - (1/4)*exp(-mcmc$mu * delta_t))*denovo(freq_1y, mcmc$p)
            ))
